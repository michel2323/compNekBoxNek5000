!=======================================================================
! Name        : pts_redistribute
! Author      : Adam Peplinski
! Version     : last modification 2015.03.09
! Copyright   : GPL
...
!     local variables
      integer ifld              ! pointer in fieldout

!     timing
      real ltime1, ltime2
      real timemaxf1, timeminf1, timemaxe1, timemine1
      real timemaxf2, timeminf2, timemaxe2, timemine2
      real timemaxr, timeminr

!     functions
      real dnekclock, glmax, glmin

!     perform single findpts and findpts_eval before and after runnig 
!     pts_rdst for timing
...
!     timing
      ...
      timemaxf1 = glmax(ltime1,1)
      timeminf1 = glmin(ltime1,1)

!     timing
      ltime1 = dnekclock()

!     I evaluate single velocity component
!     VX
      ifld = 1
...
!     timing
      ltime2 = dnekclock()
      ltime1 = ltime2 - ltime1
      timemaxe1 = glmax(ltime1,1)
      timemine1 = glmin(ltime1,1)
...
!     timing
      ltime2 = dnekclock()
      ltime1 = ltime2 - ltime1
      timemaxr = glmax(ltime1,1)
      timeminr = glmin(ltime1,1)
...
!     timing
      ltime2 = dnekclock()
      ltime1 = ltime2 - ltime1
      timemaxf2 = glmax(ltime1,1)
      timeminf2 = glmin(ltime1,1)
...
!     timing
      ltime2 = dnekclock()
      ltime1 = ltime2 - ltime1
      timemaxe2 = glmax(ltime1,1)
      timemine2 = glmin(ltime1,1)
    
!     stamp logs
      if (NIO.eq.0) then
         write(6,*)
         write(6,*) 'Point redistribution; pts_timing:'
         write(6,'(A22,g13.5,g13.5)') 'Find 1 time min/max   ', 
     $        timeminf1, timemaxf1
         write(6,'(A22,g13.5,g13.5)') 'Eval 1 time min/max   ', 
     $        timemine1, timemaxe1
         write(6,'(A22,g13.5,g13.5)') 'Find 2 time min/max   ', 
     $        timeminf2, timemaxf2
         write(6,'(A22,g13.5,g13.5)') 'Eval 2 time min/max   ', 
     $        timemine2, timemaxe2
         write(6,'(A22,g13.5,g13.5)') 'Redist. time min/max  ', 
     $        timeminr, timemaxr
         write(6,*)
      endif
...
!     prepare mapping
      call pts_map_all(mid,lptn,npts,npoints,nptimb)
...
!     real
      if (IF3D) then
         do il=1,npts
            ...
            rbuf(2,il) = pts(2,il)
            rbuf(3,il) = pts(3,il)
            rbuf(4,il) = rst(itmp+1)
            rbuf(5,il) = rst(itmp+2)
            rbuf(6,il) = rst(itmp+3)
         enddo
      else
         do il=1,npts
            itmp = (il-1)*NDIM
            rbuf(1,il) = pts(1,il)
            rbuf(2,il) = pts(2,il)
            rbuf(3,il) = rst(itmp+1)
            rbuf(4,il) = rst(itmp+2)
         enddo
      endif

!     redistribute points
      call pts_transfer(rbuf,lrbuf,ibuf,libuf,lptn,npts)
...
!     real
      if (IF3D) then
         do il=1,npts
            ...
            pts(2,il) = rbuf(2,il)
            pts(3,il) = rbuf(3,il)
            rst(itmp+1) = rbuf(4,il)
            rst(itmp+2) = rbuf(5,il)
            rst(itmp+3) = rbuf(6,il)
         enddo
      else
         do il=1,npts
            itmp = (il-1)*NDIM
            pts(1,il) = rbuf(1,il)
            pts(2,il) = rbuf(2,il)
            rst(itmp+1) = rbuf(3,il)
            rst(itmp+2) = rbuf(4,il)
         enddo
      endif
